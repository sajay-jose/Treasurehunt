# Generated by Django 4.1.3 on 2023-03-15 18:48

from django.db import migrations, models


class Migration(migrations.Migration):

    initial = True

    dependencies = [
    ]

    operations = [
        migrations.CreateModel(
            name='Game',
            fields=[
                ('id', models.BigAutoField(auto_created=True, primary_key=True, serialize=False, verbose_name='ID')),
                ('game_name', models.CharField(max_length=50)),
                ('game_id', models.CharField(max_length=50, unique=True)),
                ('game_level', models.CharField(choices=[('1', '1'), ('2', '2'), ('3', '3')], max_length=1)),
                ('clues', models.CharField(max_length=500)),
                ('start_date', models.DateTimeField(auto_now_add=True)),
                ('end_date', models.DateTimeField(auto_now_add=True)),
                ('rules', models.TextField()),
            ],
        ),
        migrations.CreateModel(
            name='User',
            fields=[
                ('uid', models.AutoField(primary_key=True, serialize=False)),
                ('name', models.CharField(max_length=50)),
                ('email', models.EmailField(max_length=254, unique=True)),
                ('password', models.CharField(max_length=128)),
                ('new_password', models.CharField(blank=True, max_length=128, null=True)),
                ('register_as', models.CharField(blank=True, choices=[('1', 'Coordinator'), ('2', 'Player')], max_length=1, null=True)),
                ('game_name', models.CharField(blank=True, max_length=50, null=True)),
                ('game_id', models.CharField(blank=True, max_length=20, null=True)),
                ('phonecode', models.CharField(max_length=10)),
                ('phone', models.CharField(max_length=20)),
                ('token', models.CharField(blank=True, max_length=64, null=True)),
                ('created_at', models.DateTimeField(auto_now_add=True)),
                ('updated_at', models.DateTimeField(auto_now=True)),
                ('deleted_at', models.BooleanField(default=False)),
            ],
        ),
    ]
